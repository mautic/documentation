...
From Mautic 2.13 tracking code support also company fields. Mautic assign company to your tracking contact based on own rules for find contact's company from  paramteres:
 - company or companyname
 - If parameters above weren't set and email or company email were set up, then try find from email or companyemail address based on domain name (company name from **david@mautic.com** is **mautic**)
 - If logice above weren't set and company website were set up, then try find from companywebsite  (company name from **www.mautic.com** is company name **mautic**)
