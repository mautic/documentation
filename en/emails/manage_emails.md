...
Embed the tokens as `{leadfield=FIELDALIAS|true}`. The |true tells Mautic to encode the value used for example in urls.

##### Date format

For custom date fields use:

- `{leadfield=DATEFIELDALIAS|datetime}`
- `{leadfield=DATEFIELDALIAS|date}`
- `{leadfield=DATEFIELDALIAS|time}`
