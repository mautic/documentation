...
Mautic currently does not have a built in way of handing double opt-ins (although it is planned). This can be achieved, though, through the use of lead lists, a custom lead field, a landing page and a campaign. To configure such a scenario, consider the following:

1. Create a lead list without filters to house the leads that need to go through the double opt in process. 
2. Create a boolean custom lead field to note if the lead has double opted in and set the default value to 0.
3. Create a landing page that will serve as the "confirmation page" for the double opt-in.
4. Create a confirmation email and insert the token for the confirmation landing page (drag and drop the specific page in the builder or insert {pagelink=ID} if using custom HTML where ID is the id of your landing page). The token will be replaced with the URL of the landing page so use it inside the instructions to click on the link for confirmation.
5. Create a campaign and assign the lead list from step 1 to it.  This campaign will automate the double opt-in dropflow.  In the Campaign Builder, start by adding the "Send email" action and select the confirmation email to be sent. Connect that into a "Visits a page" decision. You may be tempted to use the "Opens email" decision, but for this scenario, it's not the opening of an email that matters rather the lead taking the extra step to click the link to confirm. So use the "Visits a page" instead and select your confirmation landing page in the the "Limit to Pages" dropdown. Add a "Update lead" action, enter a 1 for the double opt in field, save then connect it to the green decision point of the "Visits a page" decision. Also, you may also want to add and connect a "Modify lead's lists" action to the same decision point to remove the lead from the pending confirmation lead list.
6. Now whenever you create your other lead lists, use a filter setting the double opt-in field to equal 1 so that only leads who have gone through the double opt in flow will be applicable.
