...
### ウェブサイト監視
ウェブサイトへのトラフィックすべてをモニタリングするにはトラッキングピクセルを追加するだけ十分です。 Mautic へログイン済みのユーザについてのトラフィックはモニタリングできないため，トラフィックの記録をとるという事は重要な事です。トラッキングピクセルが動作しているかどうかは incognito モード や プレイべートブラウズ，または Mautic からログアウトして確認してみてください。
